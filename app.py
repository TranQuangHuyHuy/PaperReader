...
print(f"pipeline is running on: {device}")
if device == "cuda":
...
def get_embedder():
    ...
    if embedder is None:
        ...
        embedder = SentenceTransformer(embeddingModel, trust_remote_code=True, device=device)
...
# utils
"""parse json from response llm"""
def parse_json(text):
    # delete markdown code block
...
def convert_pdf(pdf_path):
    ...
    with torch.inference_mode():
        process_document(str(pdf_path), dolphin, outputPath, max_batch_size=128)
...
def upload_chunks(chunks, collection_name):
    ...
    for i in range(0, total, batch_size):
        ...
        vectors = emb.encode(texts, batch_size=len(texts), show_progress_bar=False, convert_to_numpy=True, normalize_embeddings=True)
        # create points
        points = []
...
def search(query, collection_name, top_k=10):
    emb = get_embedder()
    client = get_qdrant()
    query_vec = emb.encode(query, normalize_embeddings=True)

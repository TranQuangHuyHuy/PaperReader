...
class DOLPHIN:
    def __init__(self, model_id_or_path):
        ...
        self.processor = AutoProcessor.from_pretrained(model_id_or_path, use_fast=True, trust_remote_code=True)
        ...
        print(f"dolphin is running on: {self.device}")
...
def process_document(document_path, model, save_dir, max_batch_size=None):
    ...
    if file_ext == '.pdf':
        ...
        with ThreadPoolExecutor(max_workers=2) as ex:
            futures = [
                ex.submit(_run_page, page_idx, pil_image)
                for page_idx, pil_image in enumerate(images)
            ]

            for f in as_completed(futures):
                page_idx, (json_path, recognition_results) = f.result()
                all_results.append({
